...
const qdoc = {key : {participant : '0@s.whatsapp.net', ...(m.chat ? { remoteJid: `status@broadcast` } : {}) },message: {documentMessage: {title: `𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐛𝐲 SunShine`, jpegThumbnail: ""}}}

const qloc = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐛𝐲 SunShine`,jpegThumbnail: ""}}}
...
switch (command) {
	case 'addown': case 'addowner': {
		...
		if(!users.length || !text) return m.reply("𝗕𝘂𝗸𝗮𝗻 𝗴𝗶𝘁𝘂 𝙠𝙖𝙠\ngini .addown nomor") 
